...
def main(args):
    ...
    for prompt_pair in dataset[:args.max_prompts]:
        for seed in range(args.num_seeds):
            ...
            for timestep in range(args.num_steps): 
                ...
                squared_norm_baseline = torch.sum(torch.sqrt((diff_baseline/10) ** 2), dim=[0,1,2])
                ...
                for layer in range(19):
                    ...
                    squared_norm = torch.sum(torch.sqrt((diff/10) ** 2), dim=[0,1,2])
    ...
    # Save results
    output_path = f'MI_metrics_{args.target}.json'
