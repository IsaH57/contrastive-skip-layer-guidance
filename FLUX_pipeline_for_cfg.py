...
class CustomFluxPipeline(FluxPipeline):
    def __init__(self, *args, layer_skip_config=None, **kwargs):
        super().__init__(*args, **kwargs)

        # (Optional) Patch transformer blocks here, or do it externally
        self._patch_transformer_for_skipping(layer_skip_config)

    def _patch_transformer_for_skipping(self, layer_skip_config):
        for i, block in enumerate(self.transformer.transformer_blocks):
            orig_forward = block.forward

            def make_wrapped_forward(index, orig_fwd):
                def wrapped_forward(self, hidden_states, encoder_hidden_states=None, *args, **kwargs):
                    if layer_skip_config is not None and index in layer_skip_config["skip_layers"]:
                        return encoder_hidden_states, hidden_states
                    return orig_fwd(hidden_states, encoder_hidden_states, *args, **kwargs)
                return wrapped_forward

            block.forward = types.MethodType(make_wrapped_forward(i, orig_forward), block)
    ...
    def __call__(
        ...
        mu = FluxPipeline.calculate_shift(
            image_seq_len,
        ...
        def noise_pred_with_skipped_layers(hidden_states, timestep, guidance, pooled_projections, encoder_hidden_states, txt_ids, img_ids, joint_attention_kwargs, return_dict):
                    ...
                    skip_layers = self.layer_skip_config.get("skip_layers", set()) if hasattr(self, "layer_skip_config") else set()
                    original_forwards = {}
