...
SKIPPED_LAYERS = [[5], [8], [12], [5,8], [5,8,12]]
GUIDANCE_SCALES = [1.5,2.,2.5]
...
results_dir = os.path.join(os.getcwd(), 'CFG_VS_SLG_experiments', f"flux_results_{timestamp}")
    ...
    for skipped_layers in SKIPPED_LAYERS:
        for gs in GUIDANCE_SCALES:
            ...
            with torch.no_grad():
                ...
                slg_path = os.path.join(subdir, f"slg_no_prompt_scale_{gs}_layers" + '_'.join(map(str, skipped_layers)) + ".png")
                image_slg.save(slg_path)
                print(f'Saved slg image to: {slg_path}')
                del image_slg
